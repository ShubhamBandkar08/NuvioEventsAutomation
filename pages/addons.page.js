export class AddonsPage {
  ...
  async pickTime(time) {
    ...
    // Select Hour (matches combobox with 'HH' exactly)
    // Using .first() because the component structure might expose multiple elements with the same role/text (e.g. wrapper, icon)
    await this.page.getByRole('combobox').filter({ hasText: /^HH$/ }).locator(':visible').first().click();
    await this.page.getByRole('option', { name: hh, exact: true }).first().click();

    // Select Minute (matches combobox with 'MM' exactly)
    // await this.page.getByRole('combobox').filter({ hasText: /^MM$/ }).locator(':visible').first().click();
  ...
  async createAddon(data) {
    ...
    const checkBox = await this.page.locator("[role$='checkbox']");
